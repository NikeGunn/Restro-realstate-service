...
## `lucky_draw` app — Phase 2 Lucky Draw

### Phase 2 — DONE (2026-06-01, 329/329 backend tests pass, frontend prod build clean)

QR-code lucky-draw campaigns mounted at **`/api/v1/lucky_draw/`** (auth) + **`/public/lucky-draw/`** (public, no auth, throttled). Scan→win→**WhatsApp coupon**→**share-for-bonus** — the flagship viral loop. Builds on Phase 0 `common` + Phase 1 `crm`. **Does NOT touch the `coupons` app** (that grants org plan tiers; lucky_draw coupons are customer discount codes — different domain).

**Models** (`models.py`, migration `0001`, tables `lucky_draw_*`, UUID PKs): `LuckyDrawCampaign` (status FSM draft/active/paused/ended, per-customer daily/total limits, `requires_*` field toggles, consent text, default_language=zh-TW, `deliver_coupon_via_whatsapp`/`referral_enabled`/`referral_bonus_type`, `coupon_validity_days`, `tag_redeemers_as_buffet`), `LuckyDrawPrize` (`discount_percent` 0.01–100 via CheckConstraint, `weight`, **denormalized `wins_today_count`/`wins_total_count`** bumped with `F()` in the draw txn so limit checks never COUNT entries), `LuckyDrawEntry` (**APPEND-ONLY core facts** — `save()` blocks post-create writes unless `update_fields ⊆ ENTRY_MUTABLE_FIELDS` status-transition whitelist; unique `coupon_code`/`referral_token`; indexes on `(campaign,phone,entered_at)`, `(campaign,ip_hash,entered_at)`, `coupon_code`, `referral_token`, `(status,expires_at)`), `LuckyDrawQRCode` (ImageField `qr_image`/`poster_image` → **default storage**, unique `url_token`, `scan_count`).

**Services** (`services/`): `draw_engine.py` (pure: `draw_prize` weighted `random.choices` over prizes with daily/total headroom; `generate_coupon_code` alphabet `23456789ABCDEFGHJKMNPQRSTUVWXYZ` — no ambiguous 0/O/1/I/L; `generate_referral_token`; `validate_entry_eligibility` — status/window/**per-phone HARD daily+total**, **per-IP SOFT** since HK shares NAT IPs). `entry_service.py` (the full atomic pipeline: eligibility → **`select_for_update` on prizes** → draw → write Entry(drawn)+unique coupon+expiry+referral_token → `F()` counter bump → CRM `get_or_create_customer`(source=lucky_draw) → consent IFF given → `lucky_draw_lead` tag → `lucky_draw_entry` interaction → referral attribution; coupon-clash retry). `qr_service.py` (QR PNG, entry URL `/public/lucky-draw/{token}/`, 1200×1800 Pillow poster zh-TW). `delivery_service.py` (🇭🇰 loop 1: `deliver_coupon_whatsapp` — **gated on `crm.consent_service.has_marketing_consent(customer,'whatsapp')`**, sets `whatsapp_sent_at`, logs `whatsapp_coupon_sent`, failure-safe; `send_expiry_reminder`). `referral_service.py` (🇭🇰 loop 2: `apply_referral` — links `referred_by_entry`, bumps `referral_count`, rejects self/same-phone, idempotent per referee phone).

**Public API** (`public_views.py`+`public_urls.py`, `AllowAny`, `authentication_classes=[]`): `GET …/{token}/config/` (`PublicBurstThrottle`, atomic `scan_count` F()-bump), `POST …/{token}/enter/` (`PublicFormThrottle`, **idempotent via Phase 0 helper + cached result replay** → returns prior payload on retry, no second draw; queues delivery on `transaction.on_commit`; response = `{won,discount_percent,coupon_code,expires_at,referral_url,share_text}` with localized zh-TW/zh-CN/en share text). `GET …/{token}/` → **Django-rendered HTML entry page** (`template_views.py` + `templates/lucky_draw/entry_form.html`, **vanilla JS, zh-TW default w/ lang switch**, animated reveal, WhatsApp-sent confirmation, share-for-bonus deep-link — standalone, never loads the React bundle).

**Auth API** (`views.py`+`urls.py`, `OrgScopeMixin`+`IsOrgMember` read/`IsOrgOwner` write): `LuckyDrawCampaignViewSet` (CRUD + `activate`/`pause`/`end` + `prizes` nested CRUD + `qr-codes` list/create + `qr-codes/{id}/poster` PNG + `entries` RO filterable + `stats` — totals, prize distribution, **WhatsApp delivery rate**, **referral funnel**). `LuckyDrawPrizeViewSet` (org-scoped via campaign, owner-only create). `LuckyDrawEntryViewSet` (RO + `redeem` action: coupon→redeemed, logs `coupon_redeemed`, tags `buffet_customer` if opted in; 404 cross-org / 409 already / 410 expired).

**Signals** (`signals.py`, lazy in `apps.py::ready()`): campaign `post_save`(created) → ensure `lucky_draw_lead`+`buffet_customer` CRM tags exist (defensive backfill; failure-safe). **Celery** (`tasks.py` + 3 beat entries): `send_coupon_whatsapp_task` (queued on_commit), `reset_prize_daily_counters_task` (00:01), `send_expiry_reminders_task` (hourly), `expire_entries_task` (00:15). **Settings**: `PUBLIC_BASE_URL` (default kribaat.com — **non-secret → belongs in k8s/configmap.yaml in prod**). **Admin**: all 4 models; entry is append-only (no add/change/delete); prizes+QR inline on campaign.

**Frontend**: `services/lucky_draw.ts` (typed client, `BASE='/v1/lucky_draw'`). Pages `pages/lucky_draw/`: `CampaignListPage` (status/WhatsApp/referral cards), `CampaignDetailPage` (tabs Overview[KPIs+prize donut+**referral funnel bar**] / Prizes[weight bars] / QR[generate/download/poster] / Entries[table+**redeem**]), `CampaignFormPage` (create/edit + **prize builder w/ live % viz** + WhatsApp/referral toggles + bonus-type). Routes in `App.tsx` (`/lucky-draw`, `/new`, `/:id`, `/:id/edit`). Sidebar: new **`engageGroup`** (`Gift` icon, pathPrefix `/lucky-draw`) after crmGroup. i18n `luckyDraw.*` + `nav.engage`/`nav.luckyDraw` in **en/zh-CN/zh-TW** (zh-TW complete). Reuses `InventoryLoading/Empty/Error` + Shadcn + recharts.

**Tests** (`apps/lucky_draw/tests/`, **62 — spec min was 26**): `test_draw_engine` (8: ambiguous-char exclusion, seeded 1000-draw distribution ±5pp, daily/total caps, uniqueness retry), `test_entry_validation` (8: per-phone HARD daily+total, per-IP SOFT, window/status), `test_public_api` (10: full pipeline creates CRM customer+consent+tag+interaction, **idempotency replay = same result no 2nd entry**, consent-not-pre-checked, 422 missing fields, counter bumps), `test_referral` (5: link+bump, self/same-phone reject, idempotent), `test_delivery` (6: **consent-gated send asserts service NOT called without consent**, sets `whatsapp_sent_at`+logs with consent, idempotent, failure-safe, campaign opt-out), `test_qr_service` (4: entry URL, valid PNG bytes, poster PNG, token uniqueness), `test_api` (14: CRUD, activate-requires-prize, transitions, prizes, QR, stats+referral funnel, redeem flow incl. cross-org 404/409/410), `test_models` (7: append-only transitions + immutable-field reject, unique coupon, discount range, signal seeds tags). **Full suite 329/329 (267 prior + 62 new) green in Docker.** Frontend `npm run build` clean; `build:check` zero new TS errors in lucky_draw files.

### Phase 2 known limitations / debt
...
- Next authorized phase is **Phase 6 (AI Credit & Usage Billing System)** — DONE (see below).

## `billing` app — Phase 6 AI Credit & Usage Billing

### Phase 6 — DONE (2026-06-03, 450/450 backend tests pass, frontend prod build clean)

The credit wallet + usage ledger + spend-cap layer Phase 5's adapter targets, mounted at **`/api/v1/billing/`** with a rich React dashboard. **NOT a payment processor** (no Stripe/cards — usage tracking + credit management only, per REQUIREMENTS § Phase 6). Builds on Phase 0 `common`. Plans are still granted via admin / the `coupons` app; this app meters usage and enforces the cap. **The Phase 5 `content_studio.services.credits` adapter now auto-delegates to `billing.credit_service` with ZERO Phase 5 code change** (it matches the adapter's exact contract: `reserve_credits`/`confirm_credits`/`refund_credits`/`get_event`).

**Models** (`models.py`, migrations `0001`+`0002_seed_plans`+`0003_backfill_balances`, tables `billing_*`, UUID PKs): `SubscriptionPlan` (admin-seeded; `plan_code` maps to `Organization.plan`; `monthly_image_credits`; `enabled_modules` JSON), `AccountSubscription` (one active/org; period + cap), **`UsageCreditBalance`** (OneToOne org — **THE row locked** via `select_for_update`; free/paid remaining + used-this-month + `reserved_credits` + `current_estimated_spend_hkd` + `cap_status`; `total_available` property = free+paid−reserved; **deduction order free→paid**; bookkeeping `reserved_free`/`reserved_paid` on the event so refunds restore the right bucket), **`UsageEvent`** (APPEND-ONLY ledger; unique `idempotency_key` → Celery-retry-safe; a refund is a NEW `refunded` row, original → `failed`; indexes on (org,-created_at)/(org,module)/status/reference_id), `UsageLimit` (OneToOne; owner-set `monthly_ai_spend_cap_hkd` + `alert_at_percent`), `MonthlyUsageSummary` (Celery rollup, unique (org,year,month)).

**Services** (`services/`):
- **`credit_service.py` — the reserve→confirm→refund saga (REQUIREMENTS § 0 #9, the highest-risk code).** Every mutation does `select_for_update` on the balance row. `reserve_credits` (idempotent on key — a retry returns the existing event, no double-reserve; raises `InsufficientCreditsError` on cap/credit block → caller maps to blocked_by_cap, no charge; splits free/paid first), `confirm_credits` (reserved→used free-first, sets actual cost, **only paid credits are billable**, recompute cap), `refund_credits` (reserved→release, or success→restore credits+back-out spend; appends refunded row; idempotent on already-failed/refunded), `reset_monthly_credits` (free reset to plan grant, **paid untouched**, period advances). **Gotcha learned:** `UsageEvent.objects.create()` needs `organization_id=` (not `organization=uuid`) — passing a UUID to the FK raises.
- `cap_monitor.py` — `recompute_cap_status(balance)`: bands `<50 active · 50–79 warning_50 · 80–99 warning_80 · ≥100 blocked` (% of cap). Notifies owner **only on an upward crossing** into a non-active band, via inventory's `_resolve_owner_phone` + `WhatsAppService.send_message` (failure-safe). Free credits never trigger blocked (only paid spend increments `current_estimated_spend_hkd`).
- `provisioning.py` — `ensure_billing_for_org` (idempotent: balance+limit+subscription; free grant from the plan matching `org.plan`). `meter.py` — `record_usage` (best-effort free `success` event for non-credit AI like inventory queries; never raises; checks billing installed).

**Signals** (`signals.py`, lazy in `apps.py::ready()`): Org `post_save`(created) → `ensure_billing_for_org` (failure-safe, never blocks org save). Backfill migration covers existing orgs.

**Celery** (`tasks.py` + 4 beat entries): `reset_monthly_credits_task` (1st 00:05), `generate_monthly_summary_task` (2nd 01:00), `check_cap_alerts_task` (every 6h), `reconcile_stale_reservations_task` (every 15min — refunds `reserved` events older than `RESERVATION_TTL_MINUTES`=15, so a crash between provider-success and confirm never leaves a silent charge). All per-org loops, failure-isolated.

**Integration** (one-directional, no core-app imports): Phase 5 `content_studio.services.credits` → `billing.credit_service` (automatic now that billing is installed). `inventory.services.ai_engine.query()` → `billing.services.meter.record_usage` (best-effort, before its return; never blocks). **Never imports billing into** messaging/restaurant/realestate.

**Settings**: `BILLING_SETTINGS` (`RESERVATION_TTL_MINUTES`, `USD_TO_HKD`=7.8, `NOTIFY_OWNER_ON_CAP`). **Admin**: all 6 models; `UsageEvent` append-only (no add/change/delete) with pretty-JSON metadata; `MonthlyUsageSummary` read-only.

**API** (`/api/v1/billing/`, OrgScopeMixin + IsOrgMember): `CreditBalanceView` (GET), `UsageSummaryView` (GET — spend/credits/cap + by-module rollup), `UsageLimitView` (GET members / **PATCH owner-only**), `UsageEventViewSet` (RO + module/status filters), `MonthlyUsageSummaryViewSet` (RO). `?organization=` resolved against membership (cross-org → 404).

**Frontend** (`pages/billing/`, rich UI built with Vercel React best-practices — Promise-free parallel single-fetch, `useMemo` chart data, derive-during-render, ternary conditionals, direct recharts imports): `BillingDashboardPage` (gradient hero with **SVG circular spend gauge**, credit-wallet stat cards, spend-vs-cap bar with 50/80% ticks, by-module recharts donut, cap-status banner), `UsageHistoryPage` (filterable + client-paginated ledger table, free/status badges), `UsageLimitsPage` (owner-only cap + alert-% form with explainer; read-only for managers), `MonthlySummaryPage` (stacked bar + 12-month table). `services/billing.ts` typed client. New sidebar **`billingGroup`** (`Wallet` icon, prefix `/billing`, 4 leaves). Routes in `App.tsx`. i18n `billing.*` (43 keys) + `nav.billing*` in en/zh-CN/zh-TW. **Also shipped this session:** `components/PlanGate.tsx` (graceful Power-plan 403 screen) wired into all Content Studio pages, fixing the raw-AxiosError-on-expired-plan bug.

**Tests** (`apps/billing/tests/`, **35 — spec minimum was 24**): `test_credit_service` (7: free-first, free→paid span, free-not-billable, insufficient blocks, cap blocks, idempotent reserve, USD→HKD), `test_saga` (6: reserve→refund nets zero, refund-after-confirm restores, double-refund idempotent, **stale reservation reconciled**, fresh not reconciled), `test_concurrency` (1, `transaction=True` + threads + Barrier: **two concurrent 8-credit reserves on a 10-credit balance → exactly one wins** via select_for_update), `test_cap_monitor` (7: 50/80/100 bands, owner-notify on upward crossing, no-notify when unchanged/downward), `test_reset` (2: free reset paid kept, task runs all orgs), `test_api` (8: balance, summary, limit GET, owner PATCH ok, **manager PATCH 403**, ledger scoped, cross-org isolation, outsider/unauth), `test_models` (4: signal bootstraps balance+limit, seeded plans present, total_available, subscription created). **Full suite 450/450 (413 prior + 35 + 2) green in Docker** — the Phase 5 Content Studio tests still pass through the now-real billing adapter. Frontend `build` clean; `build:check` zero errors in billing/PlanGate files.

### Phase 6 known limitations / debt
- `meter.py` records inventory AI queries as free `success` events for reporting only (no balance mutation/cap enforcement) — correct, since inventory AI isn't credit-metered, only Content Studio image generation goes through the reserve saga.
- `USD_TO_HKD` is a fixed settings rate (7.8); no live FX. Fine for HKD pegging.
- `current_period_end` on `AccountSubscription` is set at provision/backfill but not auto-advanced by `reset_monthly_credits_task` yet (the balance period_start IS advanced; subscription period is cosmetic until a future dunning feature needs it).
...
2. Codex responds with a brief plan (models touched, migrations needed, test plan, frontend touchpoints) and asks the user to confirm.
3. Only after confirmation, Codex implements in slices using the same pattern as Phase 2/3 (TaskCreate per slice, Docker pytest after each, frontend `npm run build:check` at the end).
4. AGENTS.md gets updated to move the implemented bullets out of "Future phases" and into a new dated `### Phase 4 — DONE` block.

If the user asks for "Phase 4" without specifying scope, Codex must NOT pick from this list arbitrarily. Ask which bullets, in which order. The list is unordered on purpose.
